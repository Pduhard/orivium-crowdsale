	...
	Pausable
{
	...
	AggregatorV3Interface internal immutable arbPriceFeed;
	...
	constructor(
		...
		require(_arb != address(0), "Crowdsale: arb token is the zero address");
		require(
			_vesting != address(0),
		...
		require(
			_arbPriceFeed != address(0),
			"Crowdsale: arb price feed is the zero address"
		);
		require(
		...
		arbPriceFeed = AggregatorV3Interface(_arbPriceFeed);
	...
	function buyTokensWithARB(uint weiAmount) public payable {
		return buyTokensWithARB(weiAmount, "0000000000");
	}

	/**
	 * @notice buy orivium token with ARB
	 *
	 * @notice This function has a non-reentrancy guard, so it shouldn't be called by
	 * 		another `nonReentrant` function.
	 */
	function buyTokensWithARB(
		uint weiAmount,
		string memory sponsorKey
	) public payable nonReentrant whenNotPaused {
		address beneficiary = _msgSender();
		...
		uint tokenAmount = getORIAmountForARB(weiAmount);
		require(
			tokenAmount >= getMinimumBuyableAmount(),
			"Crowdsale: subceed minimum amount of token buyable for this phase"
		);
		...
		arb.safeTransferFrom(_msgSender(), fundsCollector, weiAmount);
	}
	...
	/**
	 * @notice get ETH price for given wei amount in ARB
	 */
	function getARBPriceFor(uint weiAmount) public view returns (uint) {
		uint weiUsdByArb = _getLatestARBPriceFeed();
		uint usdPrice = getUSDPriceFor(weiAmount);
		return (usdPrice * (10 ** arb.decimals())) / weiUsdByArb;
	}

	/**
	 * @notice get ORI amount for given ARB wei amount
	 */
	function getORIAmountForARB(uint weiAmount) public view returns (uint) {
		uint weiUsdByArb = _getLatestARBPriceFeed();
		uint usdPrice = (weiAmount * weiUsdByArb) / (10 ** arb.decimals());
		return getORIAmountForUSD(usdPrice);
	}
	...
	function _getLatestARBPriceFeed() internal view returns (uint) {
		return _getLatestPriceFeed(arbPriceFeed);
	}
